...
#%%
for j in range(0,len(parameter_data_dec_Ni),1): #len(parameter_data) #  j=0
    cat_idx = j
    ...
    class MyProblem(ElementwiseProblem):
        ...
        def _evaluate(self, X, out, *args, **kwargs):
            ...
            result = sim_NH3_dec_Ni(link, ammonia_cost, d_km, cat_idx, T, P, GHSV, elec_cost, cooling_water_cost, catalyst_cost_Ni, ads_cost, NOxtreat_cost, BlueNH3_cost)
    ...
    # 최적 지점에서의 파라미터로 재계산
    best_result = sim_NH3_dec_Ni(link, ammonia_cost, d_km, cat_idx, T_opt, P_opt, GHSV_opt, elec_cost, cooling_water_cost, catalyst_cost_Ni, ads_cost, NOxtreat_cost, BlueNH3_cost)

    
    # 결과 저장 경로 설정
    wd = os.getcwd()
    save_dir = os.path.join(wd, f"Optimization_results_catO/Dec/Ni_T,P,GHSV")
