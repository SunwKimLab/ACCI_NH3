...
#%% file load
df = pd.read_csv('decomposition catalyst experiment data Ni.csv',encoding='CP949')
experiment_data = df
#%% 
R = 8.314 # J mol−1 K−1
...
#i = 14*(n-1)
    ...
    wd = os.getcwd()
    saving_path = wd+"\par_fit_figure/Ni"
...
###data 저장
#par
par_df.to_csv('decomposition LHHW aspen parameter Ni.csv', index=False)  
#rxn_rate_simulation
with open('decomposition rxn rate aspen Ni.csv', 'w', newline='') as file:
...
par_converted_df.to_csv('decomposition LHHW parameter Ni.csv', index=False)                
...
y=x
plt.plot(x,y,c='red')
plt.title('H$_{2}$ production rate (mol g$^{-1}$ h$^{-1}$) (Ni)',fontsize=17)
...
plt.grid(linestyle = '--')
wd = os.getcwd()
saving_path = wd+"\par_fit_figure/Ni/Parity plot (Ni)"
...
error_df.to_csv('decomposition rxn rate error Ni.csv', index=False) 


#%%

df_rxn_rate_aspen_Ni = pd.read_csv('decomposition rxn rate aspen Ni.csv',encoding='CP949',header=None)

df_rxn_rate_aspen_Ru = pd.read_csv('decomposition rxn rate aspen Ru.csv',encoding='CP949',header=None)


df_experimental_data_Ni = pd.read_csv('decomposition catalyst experiment data Ni.csv',encoding='CP949')
df_experimental_data_Ru = pd.read_csv('decomposition catalyst experiment data Ru.csv',encoding='CP949')


df_catalyst_name_Ni = pd.read_csv('decomposition catalyst name Ni_2.csv',encoding='CP949')
df_catalyst_name_Ru = pd.read_csv('decomposition catalyst name Ru_2.csv',encoding='CP949')


#%% i=10
for i in range(0, len(df_rxn_rate_aspen_Ni), 14):
    ### Plot
    T = df_experimental_data_Ni.iloc[i:i+14,8].to_numpy().reshape(-1,1)
    r_syn_exp = df_experimental_data_Ni.iloc[i:i+14,13].to_numpy().reshape(-1,1)
    fig, ax = plt.subplots(dpi=300, figsize=[7, 5.5])
    ...
    #cal
    r_syn_sim= df_rxn_rate_aspen_Ni.iloc[i:i+14,0]
    ax.plot(T, r_syn_sim, linewidth = 2.1, label='Fitted data')
    
    title_row = df_catalyst_name_Ni.iloc[int(i/14),0]
    title = f"{title_row}"
    plt.title(title,size=17)
    
    plt.grid(linestyle = '--')
    plt.xlabel("Temperature ($^\circ$C)", fontsize=17)
    plt.ylabel("H$_{2}$ production rate (mol g$^{-1}$ h$^{-1}$)", fontsize=17)
    plt.xticks(fontsize=17)
    plt.yticks(fontsize=17)
...
#%%
for i in range(0, len(df_rxn_rate_aspen_Ru), 14):
    ### Plot
    T = df_experimental_data_Ru.iloc[i:i+14,8].to_numpy().reshape(-1,1)
    r_syn_exp = df_experimental_data_Ru.iloc[i:i+14,13].to_numpy().reshape(-1,1)
    fig, ax = plt.subplots(dpi=300, figsize=[7, 5.5])
    ...
    #cal
    r_syn_sim= df_rxn_rate_aspen_Ru[i:i+14]
    ax.plot(T, r_syn_sim, linewidth = 2.1, label='Fitted data')
    
    title_row = df_catalyst_name_Ru.iloc[int(i/14),0]
    title = f"{title_row}"
    plt.title(title,size=17)
...
combined_experiment_data = pd.concat([df_experimental_data_Ni, df_experimental_data_Ru], ignore_index=True)
combined_cal = pd.concat([df_rxn_rate_aspen_Ni, df_rxn_rate_aspen_Ru], ignore_index=True)

#r2
r2_overall= r2_score(combined_experiment_data.iloc[:,13], combined_cal)
print('Overall r2:',r2_overall)

# Plot
plt.figure(figsize=(7, 5.5), dpi=300)
plt.scatter(combined_experiment_data.iloc[:,13], combined_cal,c='orange',edgecolor='orange')
x=np.linspace(min(combined_experiment_data.iloc[:,13]),max(combined_experiment_data.iloc[:,13]),100)
y=x
plt.plot(x,y,c='blue')
plt.text(0, 3.7, f'R$^2$ = {r2_overall:.3f}', color='black', fontsize=20, weight='bold')

plt.title('H$_{2}$ production rate (mol g$^{-1}$ h$^{-1}$)',fontsize=20)
...
wd = os.getcwd()
saving_path = wd+"\par_fit_figure\decomposition Parity plot"
plt.savefig(saving_path)  
plt.show()
...
# X축 데이터 (실험 데이터)
x_data_Ni = df_experimental_data_Ni.iloc[:, 8]
x_data_Ru = df_experimental_data_Ru.iloc[:, 8]

y_exp_Ni = df_experimental_data_Ni.iloc[:, 13]
y_exp_Ru = df_experimental_data_Ru.iloc[:, 13]

y_cal_Ni = df_rxn_rate_aspen_Ni
...
# Ni 계산 데이터 (선)
for i in range(0, len(y_cal_Ni), 14):
    x_segment = x_data_Ni.iloc[i:i+14]  # X축 데이터 10개
    y_segment = y_cal_Ni[i:i+14]       # Y축 데이터 10개
    ax.plot(x_segment, y_segment, color='orange', linewidth=1.0, linestyle='--', label='Fitted data' if i == 0 else "")  # 범례 중복 방지

# Ni실험 데이터 (스캐터)
ax.scatter(
    x_data_Ni, y_exp_Ni, s=30, alpha=0.6, c='blue', edgecolor='k',
    linewidth=1.5, label='Experimental data'
)


ax.set_xlabel("Temperature (°C)", fontsize=20)
ax.set_ylabel('H$_{2}$ production rate (mol g$^{-1}$ h$^{-1}$)', fontsize=20)
ax.legend(fontsize=20)
plt.xticks(fontsize=17)
plt.yticks(fontsize=17)
ax.grid(linestyle='--', alpha=0.7)
ax.set_title("Ni-based", fontsize=20)
plt.tight_layout()
wd = os.getcwd()
saving_path = wd+"\par_fit_figure\Decomposition_Ni T vs rate (org scale)"
plt.savefig(saving_path)  
plt.show()
...
# Ru 계산 데이터 (선)
for i in range(0, len(y_cal_Ru), 14):
    x_segment = x_data_Ru.iloc[i:i+14]  # X축 데이터 10개
    y_segment = y_cal_Ru[i:i+14]       # Y축 데이터 10개
    ax.plot(x_segment, y_segment, color='orange', linewidth=1.0, linestyle='--', label='Fitted data' if i == 0 else "")  # 범례 중복 방지

# Ru 실험 데이터 (스캐터)
ax.scatter(
    x_data_Ru, y_exp_Ru, s=20, alpha=0.6, c='blue', edgecolor='k',
    linewidth=1.5, label='Experimental data'
)
...
plt.tight_layout()
wd = os.getcwd()
saving_path = wd+"\par_fit_figure\Decomposition_Ru T vs rate (org scale)"
plt.savefig(saving_path) 
# Ru 그래프 출력
plt.show()
...
# Ni 그래프
fig, ax = plt.subplots(dpi=300, figsize=(5.5, 5.5))
