...
#%%
for j in range(0,len(parameter_data_syn_Fe),1): #len(parameter_data) #  j=0
    cat_idx = j
    ...
    class MyProblem(ElementwiseProblem):
        ...
        def _evaluate(self, X, out, *args, **kwargs):
            ...
            result = sim_NH3_syn_Fe(link, solar_irradiance_rich, cat_idx, T, P, GHSV)
    ...
    # 최적 지점에서의 파라미터로 재계산
    best_result = sim_NH3_syn_Fe(link, solar_irradiance_rich, cat_idx, T_opt, P_opt, GHSV_opt)



    # 결과 저장 경로 설정
    wd = os.getcwd()
    save_path = os.path.join(wd, f"Optimization_results_catX/Syn/Fe_v3/cat{j}.pkl")
    # pkl로 저장
