...
# 저장된 파일 디렉토리
wd = os.getcwd()
base_dir = os.path.join(wd, "Optimization_results_catO/Dec", f"Ru_T,P,GHSV") #Ru_T,P,GHSV{GHSV}

# 결과 저장 딕셔너리
results_dict = {}

# 범위 설정
for j in range(0, len(parameter_data_dec_Ru)):  # 필요한 범위로 수정
...
for j in range(0, len(parameter_data_dec_Ru)):  # 필요한 범위로 수정
    ...
    LCOH_op_Ru_list.append(LCOH_import)
    LCOH_op_Ru_list_catO.append(LCOH_import_catO)
...
#%%
# 전체 list를 pkl로 저장
save_path = os.path.join(wd, f"affcatcost/Dec/aff_cat_cost_dec_list_Ru_opt_cond.pkl")
...
if len(positive_indices) > 0:
    ...
    # CSV 저장
    output_csv_path = os.path.join(wd, 'affcatcost/dec/optimal_affcatcost_dec_Ru_index.csv')
...
#%% optimal LCOH 결과 저장
# numpy 배열로 변환
LCOH_op_Ru_arr = np.array(LCOH_op_Ru_list_catO)

# 양수 값의 인덱스 중 최소값을 가지는 인덱스 찾기
positive_indices = np.where(LCOH_op_Ru_arr > 0)[0]
if len(positive_indices) > 0:
    min_positive_index = positive_indices[np.argmin(LCOH_op_Ru_arr[positive_indices])]
    min_positive_value = LCOH_op_Ru_arr[min_positive_index]

    # CSV 저장
    output_csv_path = os.path.join(wd, 'affcatcost/dec/optimal_LCOH_dec_Ru_index.csv')
    with open(output_csv_path, mode='w', newline='') as file:
        writer = csv.writer(file)
...
# 온도 및 촉매 리스트 정의
T_values = range(400, 610, 10)  # 300부터 500까지 10 단위 증가
catalysts = ["Ru"]  # 사용할 촉매 종류
...
#%%% Ru: 각 온도에서 컨버젼 가장 높은 것만 추출 

temperatures = range(400, 610, 10) 

GHSV_list = [36000] # [6000, 12000, 18000, 24000, 30000, 36000]

for GHSV in GHSV_list:

    n = len(parameter_data_dec_Ru)
    ...
    catalyst_names = [cat_Ru_name.iloc[i, 0] for i in lowest_indices]
    ...
    for i in lowest_indices:
        ...
        for temp in temperatures:
            try:
                key = f"dict_dec_table_{temp}_Ru"
    ...
    ax.set_title("Ru-based", fontsize=25)
...
for GHSV in GHSV_list:
    n = len(parameter_data_dec_Ru)
    lowest_indices = np.argsort(LCOH_op_Ru_list_catO)[-n:]

    # LCOH ≤ 4.9인 값들만 컬러맵 기준으로 사용
    cut_value = 5.2
    valid_affcatcosts = [LCOH_op_Ru_list_catO[i] for i in lowest_indices if LCOH_op_Ru_list_catO[i] <= cut_value]
    ...
    for i in lowest_indices:
        ...
        if len(y_values) == len(temperatures) and all(val is not None for val in y_values):
            if LCOH_op_Ru_list_catO[i] <= cut_value:
                color = cmap(norm(LCOH_op_Ru_list_catO[i]))
            else:
                color = 'gray'
    ...
    # 범례 설정 (4.9 이하만 색상, 나머지 gray)
    handles = [
        plt.Line2D([0], [0],
                   color=(cmap(norm(LCOH_op_Ru_list_catO[i])) if LCOH_op_Ru_list_catO[i] <= cut_value else 'gray'),
                   lw=2,
                   label=f"{catalyst_names[j]}")
...
#%%
highlight_label_coords = [(475, 100), (500, 60), (500, 42), (525,35), (550, 32)]  # 원하는 좌표로 수정

for GHSV in GHSV_list:
    n = len(parameter_data_dec_Ru)
    valid_indices = [i for i in range(n) if aff_cat_cost_dec_list[i] > 0 and LCOH_op_Ru_list_catO[i] > 0]
    sorted_by_lcoh = sorted(valid_indices, key=lambda i: LCOH_op_Ru_list_catO[i])
    n_valid = len(sorted_by_lcoh)
    # sorted_by_lcoh = sorted(valid_indices, key=lambda i: aff_cat_cost_dec_list[i])
    # n_valid = len(sorted_by_lcoh)
    ...
    for i in valid_indices:
        ...
        if all(v is not None for v in y_values):
            ax.plot(temperatures, y_values, color="gray", linewidth=1.0, alpha=0.5)
    ...
    for idx, i in enumerate(quantile_indices):
        ...
        if all(v is not None for v in y_values):
            ...
            ax.plot(x1, y1, color=color, linewidth=2, alpha=0.9)
            ax.plot(x2, y2, color=color, linewidth=2, alpha=0.9)

            catalyst_name = cat_Ru_name.iloc[i, 0]
            ax.text(
            ...
            legend_elements.append(Line2D([0], [0], color=color, linewidth=3, label=catalyst_name))

    ax.set_xlabel("Temperature (°C)", fontsize=35)
    ax.set_ylabel("Lab-scale\nNH$_{3}$ Conversion (%)", fontsize=35)
    ax.set_title("Ru-based", fontsize=35)
    ax.grid(True, linestyle='--', alpha=0.6)
