...
#%% file load
parameter_data_dec_Ru = pd.read_csv('decomposition LHHW aspen parameter Ru.csv',encoding='CP949')

#%%
def sim_NH3_dec_Ru(link, ammonia_cost, d_km, cat_idx, T_C, P_bar, GHSV,
                   elec_cost, cooling_water_cost, catalyst_cost_Ru, ads_cost, NOxtreat_cost, BlueNH3_cost):
    i = cat_idx
    ...
    #driving force
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/PRE_EXP/1").Value= parameter_data_dec_Ru.iloc[i,0]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ACT_ENERGY/1").Value = parameter_data_dec_Ru.iloc[i,1]
    

    #adsorption
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_A/1/2").Value = parameter_data_dec_Ru.iloc[i,2]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_B/1/2").Value = parameter_data_dec_Ru.iloc[i,3]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_A/1/3").Value = parameter_data_dec_Ru.iloc[i,4]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_B/1/3").Value = parameter_data_dec_Ru.iloc[i,5]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_A/1/4").Value = parameter_data_dec_Ru.iloc[i,6]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_B/1/4").Value = parameter_data_dec_Ru.iloc[i,7]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_A/1/5").Value = parameter_data_dec_Ru.iloc[i,8]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_B/1/5").Value = parameter_data_dec_Ru.iloc[i,9]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_A/1/6").Value = parameter_data_dec_Ru.iloc[i,10]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_B/1/6").Value = parameter_data_dec_Ru.iloc[i,11]
    ...
    if not link.Engine.IsRunning:
        ...
        if not link.Engine.IsRunning:
            ...
            O_cat_Ru = Opex.cataylst_Ru(catalyst_use, catalyst_cost_Ru=catalyst_cost_Ru)
            ...
            OPEX = O_elec + O_cat_Ru + O_ammonia + O_NH3adsorbent + O_N2adsorbent + O_labor + O_main + O_transport + O_cool + O_NOxtreat + O_ammonia_fuel
            OPEX_catO = O_elec + O_cat_Ru + O_ammonia + O_NH3adsorbent + O_N2adsorbent + O_labor + O_main + O_transport + O_cool + O_NOxtreat + O_ammonia_fuel
            ...
            SEC_heatO = (elec_use_kWh + heat_use_kWh) / m_produced_H2 #kWh/kg
