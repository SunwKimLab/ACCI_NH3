...
#%%
for j in range(0,len(parameter_data_dec_Ru),1): #len(parameter_data) #  j=0
    cat_idx = j
    ...
    class MyProblem(ElementwiseProblem):
        def __init__(self, **kwargs):
            xl = np.array([400, 1, 100])   # T, P, GHSV  
            xu = np.array([900, 8, 72000])  # T, P, GHSV
        ...
        def _evaluate(self, X, out, *args, **kwargs):
            ...
            result = sim_NH3_dec_Ru(link, ammonia_cost, d_km, cat_idx, T, P, GHSV, elec_cost, cooling_water_cost, catalyst_cost_Ru, ads_cost, NOxtreat_cost, BlueNH3_cost)

            # 목적함수 
            f1 = np.nan_to_num(result['LCOH']['LCOH_catO'], nan=1e6, posinf=1e6, neginf=1e6)
    ...
    # 최적 지점에서의 파라미터로 재계산
    best_result = sim_NH3_dec_Ru(link, ammonia_cost, d_km, cat_idx, T_opt, P_opt, GHSV_opt, elec_cost, cooling_water_cost, catalyst_cost_Ru, ads_cost, NOxtreat_cost, BlueNH3_cost)

    
    # 결과 저장 경로 설정
    wd = os.getcwd()
    save_dir = os.path.join(wd, f"Optimization_results_catO/Dec/Ru_T,P,GHSV")
    save_path = os.path.join(save_dir, f"cat{j}.pkl")
