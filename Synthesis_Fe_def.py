...
#%% data load
parameter_data_syn_Fe = pd.read_csv('synthesis LHHW aspen parameter Fe.csv',encoding='CP949')

#%%
def sim_NH3_syn_Fe(link, solar_irradiance_rich, cat_idx, T_C, P_bar, GHSV,
                   elec_cost, cooling_water_cost, catalyst_cost_Fe, lcon):
    i = cat_idx
    ...
    # data input
    #driving force
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/PRE_EXP/1").Value= parameter_data_syn_Fe.iloc[i,0] # A_f = 100000
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ACT_ENERGY/1").Value = parameter_data_syn_Fe.iloc[i,1] #J/mol # Ea_f=100000
    
    
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_A/1/2").Value = parameter_data_syn_Fe.iloc[i,2]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_B/1/2").Value = parameter_data_syn_Fe.iloc[i,3]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_A/1/3").Value = parameter_data_syn_Fe.iloc[i,4]
    link.Tree.FindNode("/Data/Reactions/Reactions/LHHW/Input/ADS_B/1/3").Value = parameter_data_syn_Fe.iloc[i,5]
    ...
    if not link.Engine.IsRunning:
        ...
        O_cat_Fe = Opex.cataylst_Fe(catalyst_use_syn_total, catalyst_cost_Fe=catalyst_cost_Fe)/1 #교체주기 1년
        ...
        OPEX_catO = O_renew_elec + O_cool + O_cat_Fe + O_hydrogen + O_nitrogen + O_labor + O_main
