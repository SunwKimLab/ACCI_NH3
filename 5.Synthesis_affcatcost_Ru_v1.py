...
# 저장된 파일 디렉토리
wd = os.getcwd()
base_dir = os.path.join(wd, "Optimization_results_catO/Syn", f"Ru_T,P,GHSV") #Ru_T,P,GHSV36000
...
# 범위 설정
for j in range(0, len(parameter_data_syn_Ru)):  # 필요한 범위로 수정
    file_path = os.path.join(base_dir, f"cat{j}.pkl")
    if os.path.exists(file_path):
        with open(file_path, 'rb') as f:
            results_dict[j] = pickle.load(f)
        print(f"✅ cat{j}.pkl 불러오기 완료")
        LCOA_op_Ru = results_dict[j]['LCOA']['LCOA_catO'] # j=1
        LCOA_op_Ru_list.append(LCOA_op_Ru)
    else:
        print(f"⚠️ cat{j}.pkl 파일 없음")

#%% optimal LCOA 결과 저장
# numpy 배열로 변환
LCOA_op_Ru_arr = np.array(LCOA_op_Ru_list)

# 양수 값의 인덱스 중 최소값을 가지는 인덱스 찾기
positive_indices = np.where(LCOA_op_Ru_arr > 0)[0]
if len(positive_indices) > 0:
    min_positive_index = positive_indices[np.argmin(LCOA_op_Ru_arr[positive_indices])]
    min_positive_value = LCOA_op_Ru_arr[min_positive_index]

    # CSV 저장
    output_csv_path = os.path.join(wd, 'affcatcost/syn/optimal_LCOA_syn_Ru_index.csv')
...
# for j in range(0, len(parameter_data_syn_Ru)):
...
# # 전체 list를 pkl로 저장
# save_path = os.path.join(wd, f"affcatcost/syn/aff_cat_cost_syn_list_Ru_opt_cond.pkl")
...
#     # CSV 저장
#     output_csv_path = os.path.join(wd, 'affcatcost/syn/optimal_syn_Ru_index.csv')
#     with open(output_csv_path, mode='w', newline='') as file:
#         writer = csv.writer(file)
...
# 저장된 경로
load_path = os.path.join(wd, "affcatcost/syn/aff_cat_cost_syn_list_Ru_opt_cond.pkl")
...
for GHSV in GHSV_list:
    ...
    for i in lowest_indices:
        ...
        for temp in temperatures:
            try:
                key = f"dict_syn_table_{temp}_Ru"
    ...
    handles = [
        plt.Line2D([0], [0],
                   color=('gray' if aff_cat_cost_syn_list[i] < 0 else cmap(norm(aff_cat_cost_syn_list[i]))),
                   lw=2,
...
#%% Ru 49개: lab scale: 온도에 따른 전환율 & LCOA
temperatures = range(300, 510, 10)  # 300부터 500까지 10 단위 증가
...
for GHSV in GHSV_list:
    n = len(parameter_data_syn_Ru)
    lowest_indices = np.argsort(LCOA_op_Ru_list)[-n:]

    # 음수가 아닌 값만 컬러맵 기준으로 사용
    valid_LCOA = [LCOA_op_Ru_list[i] for i in lowest_indices if LCOA_op_Ru_list[i] <= 1]
    ...
    for i in lowest_indices:
        ...
        if len(y_values) == len(temperatures) and all(val is not None for val in y_values):
            if LCOA_op_Ru_list[i] <= 1:
                color = cmap(norm(LCOA_op_Ru_list[i]))
            else:
                color = 'gray'
    ...
    handles = [
        plt.Line2D([0], [0],
                   color=('gray' if LCOA_op_Ru_list[i] > 1 else cmap(norm(LCOA_op_Ru_list[i]))),
                   lw=2,
                   label=f"{catalyst_names[j]}")
...
highlight_label_coords = [(385, 8), (385, 5), (430, 3.3), (425, 1.8), (400, 0.4)]  # 수동 조정 좌표
