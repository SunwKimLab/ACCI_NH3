...
#%% file load
df = pd.read_csv('synthesis catalyst experiment data Ru.csv',encoding='CP949')
experiment_data = df
...
###data 저장
#par
par_df.to_csv('synthesis LHHW aspen parameter Ru.csv', index=False)  
#rxn_rate_simulation
with open('synthesis rxn rate aspen Ru.csv', 'w', newline='') as file:
...
par_converted_df.to_csv('synthesis LHHW parameter Ru.csv', index=False)  
...
y=x
plt.plot(x,y,c='red')
plt.title('NH$_{3}$ production rate (mol g$^{-1}$ h$^{-1}$) (Ru)',fontsize=17)
...
error_df.to_csv('synthesis rxn rate error Ru.csv', index=False) 